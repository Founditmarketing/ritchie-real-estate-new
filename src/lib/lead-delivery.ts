...
function renderHtml(lead: Lead): string {
  ...
  return `<div style="background:#050a17;padding:28px">
    <table style="max-width:560px;margin:0 auto;background:#f6f3ed;border-radius:6px;overflow:hidden">
      <tr><td style="background:#131a2b;padding:18px 24px">
        <span style="color:#f6f3ed;font:700 18px Georgia,serif">Ritchie Real Estate</span>
        <span style="color:#be2132;font:600 11px/1 -apple-system,sans-serif;letter-spacing:.18em;text-transform:uppercase;display:block;margin-top:4px">New lead</span>
      </td></tr>
      <tr><td style="padding:22px 24px"><table style="width:100%;border-collapse:collapse">${rows}</table></td></tr>
      <tr><td style="padding:0 24px 22px"><a href="${/@/.test(lead.contact) ? "mailto:" + encodeURIComponent(lead.contact) : "tel:" + lead.contact.replace(/[^\d+]/g, "")}" style="display:inline-block;background:#be2132;color:#f6f3ed;font:600 12px -apple-system,sans-serif;letter-spacing:.1em;text-transform:uppercase;text-decoration:none;padding:11px 22px;border-radius:3px">Reply to ${escapeHtml(lead.name.split(" ")[0])}</a></td></tr>
    </table>
  </div>`;
}
